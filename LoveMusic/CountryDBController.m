...
@implementation CountryDBController
-(int)initDB{
...
-(NSMutableArray *)getAllCountry{
    NSMutableArray * CountryList = [NSMutableArray arrayWithCapacity:20];
    Country * country;
    if(self.initDB){
        NSString *sql = @"select CountryName from Country";
        sqlite3_stmt * statement;
        if(sqlite3_prepare_v2(db, [sql UTF8String], -1, &statement, nil)==SQLITE_OK){
            while(sqlite3_step(statement)==SQLITE_ROW){
                country=[[Country alloc]init];
                char * countryName = (char  *)sqlite3_column_text(statement, 0);
...
-(int)addCountryToLocal:(NSString *)CountryName{
    if(self.initDB){
        sqlite3_stmt *statement;
        static char*sql = "INSERT INTO Country VALUES(?)";
        ...
        sqlite3_bind_text(statement,1, [CountryName UTF8String],-1,SQLITE_TRANSIENT);
